...
/** Fixed gravity-aligned height for flow pixel scaling (set once at range seed). */
static float s_flow_scale_height_m;
/** Last gravity-aligned radar height (flow-direct Y gating). */
static float s_last_range_h_m;
...
/**
 * BNO085 game-rotation world uses +X gravity; fusion/viewer use Y-up (-Y gravity).
 * Vector map: fusion = (bno.y, -bno.x, bno.z)  <=>  -90 deg about +Z.
 */
static struct quat fusion_bno_to_fusion_y_up_quat(void)
{
    const float hs = sinf(-M_PI_F * 0.25f);
    const float hc = cosf(-M_PI_F * 0.25f);
    return mkquat(0.0f, 0.0f, hs, hc);
}

static struct quat fusion_attitude_fusion_world(void)
{
    return qnormalize(qqmul(fusion_bno_to_fusion_y_up_quat(), fusion_measured_body_attitude()));
}
...
/** Specific-force input for kalman predict (linear accel; zero when stationary). */
static struct vec fusion_acc_spec_for_kalman(struct vec accel_body)
{
    return fusion_sensor_linear_body_locked(accel_body);
}
...
static struct vec fusion_sensor_linear_body_locked(struct vec accel_imu)
{
    ...
    case FUSION_IMU_ACCEL_GRAVITY_VECTOR: {
        const struct quat q_meas = fusion_attitude_fusion_world();
...
static void fusion_integrate_flow_direct_locked(void)
{
    ...
    if (z_g < s_cfg.range_min_m) {
        z_g = s_core.range_height_hint_m;
    }
    ...
    const float d_body_x = eff_px_x * m_per_px;
    const float d_body_y = eff_px_y * m_per_px * sp;
    const float d_body_z = eff_px_y * m_per_px * cp;

    const float (*R)[3] = (const float (*)[3])s_core.R;
    float d_world_x = R[0][0] * d_body_x + R[0][1] * d_body_y + R[0][2] * d_body_z;
    float d_world_y = R[1][0] * d_body_x + R[1][1] * d_body_y + R[1][2] * d_body_z;
    float d_world_z = R[2][0] * d_body_x + R[2][1] * d_body_y + R[2][2] * d_body_z;
...
/**
 * Flow-direct vertical: clamped radar height update on pivot Y (no EKF predict/snap).
 * Rejects single-step spikes from tilt during FB/LR; passes smooth UD motion.
 */
static void fusion_update_range_height_flow_direct_locked(void)
{
    ...
    float dh = target_h - cur_h;

    /* Tilt/outlier spikes during FB/LR can exceed 0.3 m in one step — ignore. */
    if (fabsf(dh) > 0.12f) {
        FUSION_DBG("[SKIP Y DIRECT] Height innovation outlier: dh=%.3fm\n", dh);
        return;
    }

    /* Limit per-step vertical motion (~10 ms) for stability. */
    ...
    struct vec sensor_linear_world)
{
    ...
    if (s_cfg.flow_direct_position) {
        ...
        /* Absolute gravity-aligned height from radar (EKF Y); do not re-zero to origin. */
        pose.position_m.y = st.position.y;
...
static void fusion_step_locked(int64_t now_us)
{
    ...
    if (s_cfg.require_range && s_in.range_m >= s_cfg.range_min_m) {
        ...
        if (!s_height_seeded) {
            ...
            s_flow_scale_height_m = target_h;
            s_last_range_h_m = target_h;
        }
    } else {
    ...
    if (s_cfg.require_range) {
        if (s_cfg.flow_direct_position) {
            fusion_update_range_height_flow_direct_locked();
        } else {
    ...
    FUSION_DBG("[SUPERVISOR CLAMP] State out of bounds — clamping. Pos: (%.2f, %.2f, %.2f), Vel: (%.2f, %.2f, %.2f)\n",
        ...
        kalmanSupervisorClampState(&s_core);
    }
...
void fusion_submit_imu_quat(float w, float x, float y, float z, int64_t timestamp_us)
{
    ...
    if (!s_attitude_aligned) {
        fusion_apply_attitude_locked(fusion_attitude_fusion_world());
        s_attitude_aligned = true;
    }
